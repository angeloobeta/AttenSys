...
export async function getCompiledCode(filenameA: string, filenameB: string, filenameC: string, filenameD : string, filenameK: string, nftfile : string) {
    ...
    const CcasmFilePath = path.join(
        ...
        const DcasmFilePath = path.join(
            ...
            const KcasmFilePath = path.join(
                ...
                const nftSierraFilePath = path.join(
                    __dirname,
                    `../target/dev/${nftfile}.contract_class.json`
                    );
                const nftcasmFilePath = path.join(
                    __dirname,
                    `../target/dev/${nftfile}.compiled_contract_class.json`
                    );
    ...
    const codeNFT = [nftSierraFilePath, nftcasmFilePath].map(async (filePath) => {
        const file = await fs.readFile(filePath);
        return JSON.parse(file.toString("ascii"));
    });
    const [NFTsierraCode, NFTcasmCode] = await Promise.all(codeNFT);


    return {
        AsierraCode, AcasmCode, BsierraCode, BcasmCode, CsierraCode, CcasmCode,DsierraCode, DcasmCode, KsierraCode, KcasmCode,NFTsierraCode, NFTcasmCode
    };
}
