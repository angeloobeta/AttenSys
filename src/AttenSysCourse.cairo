...
pub trait IAttenSysCourse<TContractState> {
    ...
    fn add_replace_course_content(ref self: TContractState, course_identifier: u256, owner_: ContractAddress, new_course_uri_a: felt252, new_course_uri_b: felt252);
    //untested
    fn finish_course_claim_certification(ref self: TContractState, course_identifier: u256);
    //untested
    fn check_course_completion_status_n_certification(self:@TContractState, course_identifier: u256, candidate: ContractAddress) -> bool;
    fn get_course_infos(self: @TContractState, course_identifiers : Array<u256>) -> Array<AttenSysCourse::Course>;
    //untested
    ...
    fn get_all_creator_courses(self: @TContractState, owner_: ContractAddress)->Array<AttenSysCourse::Course>;    
...
mod AttenSysCourse {
    ...
    impl IAttenSysCourseImpl of super::IAttenSysCourse<ContractState> {
        fn create_course(ref self: ContractState, owner_: ContractAddress, accessment_: bool) {             
            ...
            self.specific_course_info_with_identifer.entry(current_identifier).write(course_call_data);
            self.identifier_tracker.write(current_identifier);
        }


        //from frontend, the idea will be to obtain the previous uri, transfer content from the previous uri to the new uri
        // and write the new uri to state.
        fn add_replace_course_content(ref self: ContractState, course_identifier: u256, owner_: ContractAddress, new_course_uri_a: felt252, new_course_uri_b: felt252){
            let mut current_course_info: Course = self.specific_course_info_with_identifer.entry(course_identifier).read();
            ...
            let call_data : Uri = Uri {
                first : new_course_uri_a,
                second : new_course_uri_b,
            };                 
            current_course_info.uri = call_data;        
            self.specific_course_info_with_identifer.entry(course_identifier).write(current_course_info);

            //run a loop to check if course ID exists in all course info vece, if it does, replace the uris.
        ...
        fn finish_course_claim_certification(ref self: ContractState, course_identifier: u256){
            ...
            //todo : verifier check, get a value from frontend, confirm the hash if it matches with what is being saved. goal is to avoid fraudulent course claim.
        ...
        fn check_course_completion_status_n_certification(self:@ContractState, course_identifier: u256, candidate: ContractAddress) -> bool{
            self.completion_status.entry((candidate,course_identifier)).read()
        }

        fn get_course_infos(self : @ContractState, course_identifiers : Array<u256>) -> Array<Course>{
